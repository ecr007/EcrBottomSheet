...
public struct EcrBottomSheet {
    ///The options to adjust the behavior and the settings of the EcrBottomSheet.
    public enum Options: Equatable {
        public static func == (lhs: EcrBottomSheet.Options, rhs: EcrBottomSheet.Options) -> Bool {
            return lhs.rawValue == rhs.rawValue
        }
        
        ///Allows the EcrBottomSheet to move when dragging the mainContent. Do not use if the mainContent is packed into a ScrollView.
        case allowContentDrag
        ///Sets the animation for opening and closing the EcrBottomSheet.
        case animation(Animation)
        ///The mainView is packed into a ScrollView, which can only scrolled at the .top position.
        case appleScrollBehavior
        ///Changes the background of the EcrBottomSheet. Must be erased to AnyView.
        case background(AnyView)
        ///Enables and sets the blur effect of the background when pulling up the EcrBottomSheet.
        case backgroundBlur(effect: UIBlurEffect.Style = .systemThinMaterial)
        ///Changes the corener radius of the EcrBottomSheet.
        ...
        ///Hides the drag indicator and prevents the EcrBottomSheet from being dragged.
        case notResizeable
        ///Adds a shadow to the background of the EcrBottomSheet.
        ...
        ///Dismisses the EcrBottomSheet when swiped down.
        case swipeToDismiss
        ///Dismisses the EcrBottomSheet when the background is tapped.
        case tapToDissmiss
...
internal extension Array where Element == EcrBottomSheet.Options {
    var allowContentDrag: Bool {
        return self.contains(EcrBottomSheet.Options.allowContentDrag)
    }
    ...
    var appleScrollBehavior: Bool {
        return self.contains(EcrBottomSheet.Options.appleScrollBehavior)
    }
    ...
    var backgroundBlur: Bool {
        return self.contains(EcrBottomSheet.Options.backgroundBlur())
    }
    ...
    var noBottomPosition: Bool {
        return self.contains(EcrBottomSheet.Options.noBottomPosition)
    }
    
    var noDragIndicator: Bool {
        return self.contains(EcrBottomSheet.Options.noDragIndicator)
    }
    
    var notResizeable: Bool {
        return self.contains(EcrBottomSheet.Options.notResizeable)
    }
    ...
    var showCloseButton: Bool {
        return self.contains(EcrBottomSheet.Options.showCloseButton())
    }
    ...
    var swipeToDismiss: Bool {
        return self.contains(EcrBottomSheet.Options.swipeToDismiss)
    }
    
    var tapToDismiss: Bool {
        return self.contains(EcrBottomSheet.Options.tapToDissmiss)
    }
}
